...
object DumbWithArgsConfigSessionZManagedVaultSparkJob extends App {
  ...
  } yield runtime.ContextV2(cfg, runtime.Args.tupled(arguments), spark, vault))
    .use(ctx => {
      Task {
        val verticaOpts = ctx.vault.logical().read("etl/dbs/vertica").getData.asScala
